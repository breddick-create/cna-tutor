...
import { PretestDomainSection } from "@/components/pretest/pretest-domain-section";
...
import { buildPretestResultsViewModel } from "@/lib/onboarding/pretest-results";
...
export default async function WrittenPretestResultsPage() {
  ...
  const score = getWrittenPretestScore(viewer.user);
  const breakdown = getWrittenPretestDomainBreakdown(viewer.user);

  if (score === null || breakdown.length === 0) {
    redirect("/written/pretest");
  }

  const results = buildPretestResultsViewModel({ score, breakdown });

  return (
    <div className="space-y-8">
      <section className="panel-strong rounded-[1.75rem] p-6 sm:p-8">
        <p className="eyebrow">Written Pre-Test Results</p>
        <h1 className="mt-3 max-w-4xl text-3xl font-semibold leading-tight sm:text-4xl">
          Your written pre-test is complete. Now you have a clear plan for what to study next.
        </h1>
        <p className="text-muted mt-4 max-w-3xl leading-7">
          This score is a starting point, not a final judgment. It shows where you are strongest
          across the 8 written exam domains, where you need the most support, and what to work on
          first.
        </p>
        <p className="text-muted mt-3 max-w-3xl text-sm leading-7">
          Your next move is simple: open the written study plan and start with the first topic in
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="panel rounded-[1.75rem] p-6">
          ...
          <p className="text-muted mt-3 text-sm leading-6">
            Based on your first pre-test across the Texas CNA written exam domains.
          </p>
        </div>
        ...
        <div className="panel rounded-[1.75rem] p-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-semibold">
                Open your written study plan and start with the first topic.
              </h2>
            </div>
            <span
          ...
          <p className="mt-4 text-sm font-medium leading-6">{results.readiness.summary}</p>
          <p className="text-muted mt-3 text-sm leading-6">{results.readiness.encouragement}</p>
          <div className="mt-6">
            <Link className="button-primary" href="/written/study-plan">
              Start your written study plan
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <PretestDomainSection
          emptyMessage="No standout strength areas were detected yet. That is okay. The written study plan below will help you build them."
      ...
      <section className="panel rounded-[1.75rem] p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="eyebrow">Written Study Plan Preview</p>
            <h2 className="mt-3 text-2xl font-semibold">
              Here is how your personalized written plan begins.
            </h2>
            ...
            >
              ...
              {item.estimatedMinutes ? (
                <p className="mt-2 text-sm leading-6">
                  <span className="font-semibold">Estimated time:</span> {item.estimatedMinutes}{" "}
                  minutes
                </p>
              ) : null}
