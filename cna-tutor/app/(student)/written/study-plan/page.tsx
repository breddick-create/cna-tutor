...
export default async function WrittenStudyPlanPage({
  searchParams,
}: {
    ...
    <div className="space-y-8">
      <section className="panel rounded-[1.75rem] p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="eyebrow">{text("Written Exam Prep", "Preparacion para el examen escrito")}</p>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold">
              {text(
                "Domains ranked weak to strong from your pre-test.",
                "Dominios ordenados de debil a fuerte desde tu preevaluacion.",
              )}
            </h2>
          ...
          {adaptivePlan.priorityCategories.map((section, index) => (
            ...
            >
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <Link className="button-primary w-full sm:w-auto" href="/written/study">
                  {text("Guided lesson", "Leccion guiada")}
                </Link>
                <Link
                  className="button-secondary w-full sm:w-auto"
                  href={`/written/quiz?domain=${section.slug}`}
                >
                  {text("Quiz", "Quiz")}
                </Link>
                <Link
                  className="button-secondary w-full sm:w-auto"
                  href={`/written/mock-exam?domain=${section.slug}`}
                >
      ...
      <section className="panel rounded-[1.75rem] p-6">
        <p className="eyebrow">{text("Practice Exams", "Examenes de practica")}</p>
        <h2 className="mt-2 text-2xl font-semibold">
          {text("Test your written knowledge.", "Pon a prueba tu conocimiento escrito.")}
        </h2>
        <p className="text-muted mt-3 max-w-3xl text-sm leading-6">
          {text(
            "Take a full 60-question written practice exam or a shorter 10-question quiz on any domain. These simulate the Prometric written exam format.",
            "Toma un examen completo de practica de 60 preguntas o un quiz mas corto de 10 preguntas sobre cualquier dominio. Estos simulan el formato del examen escrito de Prometric.",
          )}
        </p>
        <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          <Link className="button-primary w-full sm:w-auto" href="/written/mock-exam">
            {text("Full practice exam", "Examen completo de practica")}
          </Link>
          <Link className="button-secondary w-full sm:w-auto" href="/written/quiz">
            {text("Quick quiz", "Quiz rapido")}
          </Link>
        </div>
      </section>
    </div>
