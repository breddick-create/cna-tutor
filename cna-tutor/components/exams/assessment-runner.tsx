...
function getInlineResultContent(args: {
  ...
  if (mode === "quiz") {
    const studyHref = domainSlug ? `/study-plan?topics=${domainSlug}` : "/study-plan";
    const sectionMockHref = domainSlug ? `/mock-exam?domain=${domainSlug}` : "/mock-exam";
...
export function AssessmentRunner({
  ...
  return (
    <div className="space-y-6 pb-28 md:pb-0">
      <BadgeCelebration badges={newBadges} storageKey={`assessment-badges-${mode}-${domainSlug ?? "all"}`} />
        ...
        <section className="space-y-6">
          <div className="panel rounded-[1.75rem] p-5 sm:p-6">
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              <Link className="button-primary w-full sm:w-auto" href={inlineResultContent?.primaryAction.href ?? "/study-plan"}>
                {inlineResultContent?.primaryAction.label ?? t({ en: "Continue", es: "Continuar" })}
              </Link>
              <Link className="button-secondary w-full sm:w-auto" href={inlineResultContent?.secondaryAction.href ?? "/dashboard"}>
