...
import type { ProgressionConfig } from "@/lib/ccma/progression/config";
import { DEFAULT_PROGRESSION_CONFIG } from "@/lib/ccma/progression/config";
import { getLessonByDomain, listTutorLessons } from "@/lib/ccma/tutor/lessons";
  ...
  if (args.quizzesCompleted < 2) {
    return {
      ...
      href: args.weakestDomain?.domainSlug
        ? `/ccma/quiz?domain=${args.weakestDomain.domainSlug}`
        : "/ccma/quiz",
    };
  }
...
export function buildReadinessChecklist(args: {
  ...
  if (weakestDomain) {
    ...
    items.push({
      ...
      href: `/ccma/quiz?domain=${weakestDomain.domainSlug}`,
      met: quizMet,
    });
  }
  ...
  items.push({
    ...
    href: weakestDomain?.domainSlug ? `/ccma/quiz?domain=${weakestDomain.domainSlug}` : "/ccma/quiz",
    met: recentQuizMet,
  });
